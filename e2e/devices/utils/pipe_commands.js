...
const escapeInQuotedRegexp = (fragment) =>
    fragment.replace(SPECIAL_CHARS, '\\$1');

function win32Implementation() {
    const searchRegexpWin32 = (pattern) =>
        `findstr /R /C:"${escapeInQuotedString(pattern)}"`;
    const searchFragmentWin32 = (fragment) =>
        `findstr /C:"${escapeInQuotedString(fragment)}"`;
...
function nixImplementation() {
    const searchRegexpNix = (pattern) =>
        `grep "${escapeInQuotedString(pattern)}"`;
    const searchFragmentNix = (fragment) =>
        `grep -e "${escapeInQuotedString(fragment)}"`;
